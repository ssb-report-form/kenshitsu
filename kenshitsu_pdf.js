...
function _buildPdfFullHtml(center, date, staff, sampling, items, doPrint) {
  ...
  function buildCard(item) {
    ...
    h += '<div style="display:flex;gap:2mm;height:38mm;overflow:hidden;">' + photosHtml + '</div>';
    h += '</div></div>';
    return h;
  }
  ...
  if (defectItems.length > 0) {
    ...
    defectItems.forEach(function(item) {
      ...
      for (var p = 0; p < 2; p++) {
        if (dp[p]) {
          pagesHtml += '<div style="flex:1;height:38mm;border:1px solid #ccc;border-radius:3px;overflow:hidden;"><img src="' + dp[p] + '" style="width:100%;height:100%;object-fit:contain;display:block;background:#f9f9f9;"></div>';
        } else {
          pagesHtml += '<div style="flex:1;height:38mm;border:1px dashed #ccc;border-radius:3px;display:flex;align-items:center;justify-content:center;font-size:8px;color:#aaa;">写真' + (p+1) + '</div>';
        }
      }
      pagesHtml += '</div>';
      pagesHtml += '</div></div>';
